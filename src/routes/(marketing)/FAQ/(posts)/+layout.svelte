<script lang="ts">
  ...
  import { sortedFAQPosts, type FAQPost } from "./../posts"
  import { WebsiteName } from "../../../../config"

  function getCurrentPost(url: string): FAQPost {
    let searchPost: FAQPost | null = null
    ...
    if (!searchPost) {
      error(404, "FAQ post not found")
    }
    return searchPost
  }
  $: currentPost = getCurrentPost($page.url.pathname)

  function buildLdJson(post: FAQPost) {
    return {
